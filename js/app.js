...
//定义类Plane（飞机的构造函数）
function Plane(x, y, width, height, planeStep) {
  // GameElement.call(this, potions);//调用类GameElement的属性
  // this.potions = potions || {};
  this.x = x; //飞机的x坐标
  this.y = y; //飞机的y坐标
  this.width = width; //飞机的宽度
  this.height = height; //飞机的高度
  this.planeStep = planeStep; //飞机的移动步伐
}
...
//定义类子弹
function Bullet(x, y,width, exists) {
  this.x = x + width/2;
  this.y = y - 10;
  this.width = width;
  this.exists = exists;
...
//怪兽x和y方向的move方法
Enemy.prototype.move = function(Sx, Sy) {
    this.x += Sx;
    this.y += Sy;
}
...
function update() {
  ...
  function isDown (enemy, index, array) {
    return(enemy.x > canvas.width - 30 - enemy.width || enemy.x < 30);
  }
  ...
  if (enemies.some(isDown)) {
    while (e--) {
      ...
      //达到边界后向下移动
        ...
        if (enemy.y >= 480) {
          const gameInfoText = document.querySelector('.score');
          context.clearRect(0, 0, canvas.width, canvas.height);
...
var GAME = {
  ...
  bindEvent: function() {
    ...
    var successrePlayBtn = document.querySelector('.js-replay1');
    // 开始游戏按钮绑定
  ...
  play: function() {
    ...
    var _this = this;
    var Plane1 = new Plane(320, 480, 60, 100, 5);

    for(var l = 0; l < 2; l++) {
      
      var initY = l*50 + 30;
      // var initZ = 35 + (n-14)*(50 + 10);
      for (var n = 0; n < 7; n++) {
        var initX = 35 + n*(50 + 10);
      _this.enemyElements.push(new Enemy(initX, initY, 50, 50));
    }
    
      // else if (6 < n < 14) {
      // _this.enemyElements.push(new Enemy(initY, 80, 50, 50));
      // }
      // else if (13 < n < 21) {
      // _this.enemyElements.push(new Enemy(initZ, 130, 50, 50));
      // }
    }
